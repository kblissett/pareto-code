...
const METRICS = {
  ...
  model_terminal_bench: { scoreKey: "model_terminal_bench_score", label: "Terminal-Bench v4 · AA model evaluation", shortLabel: "Model Terminal-Bench", chartShort: "Model TB" },
  scicode: { scoreKey: "scicode_score", label: "SciCode · AA model evaluation", shortLabel: "SciCode", chartShort: "SciCode" },
  coding_agent_deep_swe: { scoreKey: "coding_agent_deep_swe_score", label: "DeepSWE v1.1 · Coding Agent Index run", shortLabel: "Agent DeepSWE", chartShort: "Agent DeepSWE" },
  coding_agent_terminal_bench: { scoreKey: "coding_agent_terminal_bench_score", label: "Terminal-Bench v4 · Coding Agent Index run", shortLabel: "Agent Terminal-Bench", chartShort: "Agent TB" },
...
function renderTable(models, plottedCount) {
  ...
  rows.innerHTML = models.map((model) => `<tr tabindex="0" aria-selected="${state.activeId === model.id}" aria-label="Inspect ${escapeHtml(model.name)} on the chart" data-row-id="${escapeHtml(model.id)}" class="${state.activeId === model.id ? "selected-row" : ""}">
    ...
    <td class="number score agent-eval" title="DeepSWE v1.1 from the selected Coding Agent Index model–agent–effort row">${scoreText(model.coding_agent_deep_swe_score)}</td>
    <td class="number score agent-eval" title="Terminal-Bench v4 from the selected Coding Agent Index model–agent–effort row">${scoreText(model.coding_agent_terminal_bench_score)}</td>
    ...
    <td class="number score model-eval" title="Artificial Analysis Coding Index: one-half model Terminal-Bench v4 plus one-half SciCode">${scoreText(model.coding_index)}</td>
    <td class="number score model-eval" title="Terminal-Bench v4 from Artificial Analysis's model evaluation">${scoreText(model.model_terminal_bench_score)}</td>
